...
export default function StyleClone() {
  const [selectedSize, setSelectedSize] = useState('ig-portrait');
  ...
  const handleGenerate = () => {
    ...
    if (selectedSize === 'custom') {
      sizeText = customSizeInput || '1080 × 1350';
    }

    let safeZoneText = '';
    if (selectedSize === 'ig-portrait' || sizeText === '1080 × 1350') {
      safeZoneText = `Rasio 4 : 5
...
Tidak boleh ada elemen penting keluar dari safe area.`;
    } else if (selectedSize === 'ig-square' || sizeText === '1080 × 1080') {
      safeZoneText = `Rasio 1 : 1

---

## Safe Zone 1:1

Jika output 1080 × 1080

Tetap gunakan margin aman sekitar 10%.`;
    } else {
      safeZoneText = `Tetap gunakan margin aman sekitar 10%.`;
    }
...
COPYWRITINGNYA:
${copywriting || '[Masukkan Copywriting Anda di sini]'}
${notesSection}`;
    ...
    <div className="pt-32 md:pt-48 pb-24 px-6 max-w-4xl mx-auto min-h-screen">
      ...
      >
        ...
        <h1 className="font-serif italic text-[42px] md:text-[80px] leading-[0.9] tracking-[-0.04em] text-[#2b2b2b]">
          Generator Prompt AI untuk Meniru Desain Poster
        </h1>
        <p className="font-sans text-[14px] md:text-[16px] leading-[1.6em] text-[#2b2b2bcc] max-w-2xl">
          Buat prompt AI untuk menghasilkan desain poster yang mengikuti gaya visual referensi dengan tingkat kemiripan tinggi. Cukup masukkan copywriting, lalu sistem akan menggabungkannya ke dalam prompt master sehingga siap digunakan pada AI Image Generator.
        </p>
        ...
        <form className="flex flex-col gap-8" onSubmit={(e) => { e.preventDefault(); handleGenerate(); }}>
          <div className="flex flex-col gap-3">
            ...
            <textarea 
              ...
              className="w-full bg-white border border-black/10 rounded-[12px] p-4 text-[15px] text-[#2b2b2b] placeholder:text-black/30 focus:outline-none focus:border-black/30 focus:ring-1 focus:ring-black/30 transition-all min-h-[120px] resize-y"
              placeholder="Masukkan copywriting lengkap yang akan digunakan pada poster..."
            />
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <textarea 
              ...
              className="w-full bg-white border border-black/10 rounded-[12px] p-4 text-[15px] text-[#2b2b2b] placeholder:text-black/30 focus:outline-none focus:border-black/30 focus:ring-1 focus:ring-black/30 transition-all min-h-[100px] resize-y"
              placeholder="Tambahkan detail spesifik, elemen yang ingin dihindari, atau instruksi khusus..."
            />
          </div>
